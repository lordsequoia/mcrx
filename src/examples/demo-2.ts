...
const startDemo = (rootDir: string) => {
    ...
    const {files$, logs$, loggedEvents$, playerEvents$, chatEvents$} = mcrx(rootDir)

    const {playerJoined$, playerLeft$} = playerEvents$
    const {messageSent$} = chatEvents$

    logs$.subscribe(v => console.debug(`# -> [${v.timestamp}] ${v.content}`))
    loggedEvents$.stream$.subscribe(v => console.warn(`E -> [${v.line.timestamp}] {${v.type}} ${JSON.stringify(v.data)}`))
